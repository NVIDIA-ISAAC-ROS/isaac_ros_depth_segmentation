...
template<>
inline constexpr uint8_t GetChannelSize<gxf::VideoFormat::GXF_VIDEO_FORMAT_GRAY32>()
{
  return 4;
}
...
template<>
inline const char * GetColorName<gxf::VideoFormat::GXF_VIDEO_FORMAT_GRAY32>()
{
  return "gray";
}
...
gxf::Expected<void> AllocateVideoBuffer(
  gxf::Handle<gxf::VideoBuffer> video_buffer, size_t width,
  size_t height, gxf::MemoryStorageType memory_storage_type,
  gxf::Handle<gxf::Allocator> allocator)
{
  if (width % 2 != 0 || height % 2 != 0) {
    GXF_LOG_ERROR(
      "Image width and height must be even, but received width: %zu, height: %zu",
      width, height);
...
#endif  // NVIDIA_ISAAC_ROS_EXTENSIONS_BI3D_POSTPROCESSOR_UTILS_HPP_
