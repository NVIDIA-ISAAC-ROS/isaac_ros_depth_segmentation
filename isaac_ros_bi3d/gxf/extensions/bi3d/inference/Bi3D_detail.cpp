...
void _pad(Tensor_CHW_C1_F32& dst, const Tensor_CHW_C1_F32& src,
    ...
    //      oDstSizeROI         Size (width, height) of the destination region, i.e. the region
    //                          that gets filled with data from the source image (inner part)
    //                          and constant border color (outer part).
    //      nTopBorderHeight    Height (in pixels) of the top border. The number of pixel rows at
    //                          the top of the destination ROI that will be filled with the
    //                          constant border color. nBottomBorderHeight = oDstSizeROI.height -
    //                          nTopBorderHeight - oSrcSizeROI.height.
    //      nLeftBorderWidth    Width (in pixels) of the left border. The width of the border at
    //                          the right side of the destination ROI is implicitly defined by the
    //                          size of the source ROI: nRightBorderWidth = oDstSizeROI.width -
    //                          nLeftBorderWidth - oSrcSizeROI.width.
    //      nValue              The pixel value to be set for border pixels for single channel
    //                          functions.
    ...
    nppiCopyConstBorder_32f_C1R_Ctx(
        ...
        topBorderHeight, leftBorderWidth, 0.0, cvcore::tensor_ops::GetNppStreamContext(stream));
}

void _threshold(Tensor_CHW_C1_F32& dst, const Tensor_CHW_C1_F32& src,
    ...
    //      pDst                Destination-Image Pointer.
    //      nDstStep            Destination-Image Stride in bytes.
    //      oSizeROI            Size (width, height) of the destination region, i.e. the region
    //                          that gets filled with data from the source image
    ...
    nppiThreshold_LTValGTVal_32f_C1R_Ctx(
        ...
        cvcore::tensor_ops::GetNppStreamContext(stream));
}
...
void _clear(Tensor_CHW_C1_F32& dst, cudaStream_t stream) {
    ...
    //      pDst            Destination-Image Pointer.
    //      nDstStep        Destination-Image Stride in bytes.
    //      oSizeROI        Size (width, height) of the destination region, i.e. the region that
    //                      gets filled with nValue.
    //      nppStreamCtx    NPP Application Managed Stream Context.
    nppiSet_32f_C1R_Ctx(
        0.0,
        static_cast<float*>(dst.getData()),
...
void _sigmoid(Tensor_CHW_C1_F32& dst, Tensor_CHW_C1_F32& src,
              cudaStream_t stream) {
    auto context = cvcore::tensor_ops::GetNppStreamContext(stream);
